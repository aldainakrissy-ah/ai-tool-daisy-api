...
public class ClientService {
    ...
    public List<Client> findAllClientsFromAllSchemas() {
        ...
        try {
            // Check if entity manager is available and connection is healthy
            if (entityManager == null) {
                logger.warn("EntityManager is null - second database may not be available");
                return List.of();
            }
            ...
            logger.warn("Database connection failure - second database may not be available: {}", e.getMessage());
            ...
            // For other exceptions, still return empty list to prevent application failure
            logger.warn("Returning empty list due to database error to maintain application stability");
            return List.of();
        }
    }
    ...
    public List<ClientGidWithClientInfo> findAllClientGidsWithClientInfoFromAllSchemas() {
        ...
        try {
            ...
            if (schemas.isEmpty()) {
                logger.warn(
                        "No schemas found containing both 'client_gids' and 'clients' tables. Returning empty list.");
                return List.of();
            }
            logger.info("Found {} schemas containing both 'client_gids' and 'clients' tables: {}", schemas.size(),
                    schemas);
            ...
            String unionQuery = schemas.stream()
                    .map(schema -> "SELECT CAST(c.id AS TEXT) as client_id, CAST(c.email AS TEXT), CAST(c.first_name AS TEXT), CAST(c.family_name AS TEXT), CAST(cg.id AS TEXT), CAST(cg.professional_id AS TEXT), CAST(c.client_gid_id AS TEXT) FROM "
                            + schema + ".clients c LEFT JOIN " + schema
                            + ".client_gids cg ON CAST(TRIM(CAST(c.client_gid_id AS TEXT)) AS BIGINT) = cg.id")
                    .collect(Collectors.joining(" UNION ALL "));
            ...
            return result.stream()
                    .map(row -> {
                        try {
                            ...
                            logger.error("Error mapping row to ClientGidWithClientInfo. Row data: {}",
                                    java.util.Arrays.toString(row), e);
                            return null;
                        }
                    })
