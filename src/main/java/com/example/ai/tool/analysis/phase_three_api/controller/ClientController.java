...
public class ClientController {
    ...
    @GetMapping("/info")
    @Operation(summary = "Get all client_gids with client info from all schemas",
               description = "Retrieves a comprehensive list of all client_gids with client info by searching through all available schemas in the second database for the 'client_gids' and 'clients' tables and consolidating the results.",
               responses = {
                   @ApiResponse(responseCode = "200", description = "Successfully retrieved the list of client_gids with client info"),
                   @ApiResponse(responseCode = "500", description = "Internal server error")
               })
    public ResponseEntity<List<ClientGidWithClientInfo>> getAllClientGidsWithClientInfo() {
        List<ClientGidWithClientInfo> clients = clientService.findAllClientGidsWithClientInfoFromAllSchemas();
        return ResponseEntity.ok(clients);
    }
}
